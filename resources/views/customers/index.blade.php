        ...
        <div class="d-flex gap-2">
            <button class="btn-cl-outline" id="archivedTabBtn" onclick="toggleView('archived')">
                ...
                @if($archivedCustomers->total() > 0)
                    <span class="badge ms-1" style="background:#f3f4f6;color:#374151;font-size:0.7rem;">{{ $archivedCustomers->total() }}</span>
                @endif
            </button>
            ...
            <div class="d-flex flex-wrap gap-2 align-items-center">
                ...
                <div class="d-flex gap-1">
                    <a href="{{ route('customers', array_merge(request()->except(['balance','page']), ['search' => $search])) }}"
                       class="cl-filter-pill {{ !($balance ?? null) ? 'active-all' : '' }}">
                        All
                    </a>
                    <a href="{{ route('customers', array_merge(request()->except(['balance','page']), ['search' => $search, 'balance' => 'with'])) }}"
                       class="cl-filter-pill {{ ($balance ?? null) === 'with' ? 'active-danger' : '' }}">
                    ...
                    <a href="{{ route('customers', array_merge(request()->except(['balance','page']), ['search' => $search, 'balance' => 'without'])) }}"
                       class="cl-filter-pill {{ ($balance ?? null) === 'without' ? 'active-success' : '' }}">
        ...
        <div class="cl-card mt-3" style="padding:0;overflow:hidden;">
                ...
                <span class="fw-bold" style="font-size:0.9rem;">
                    ...
                    @if($search || ($balance ?? null))
                        <span class="text-muted fw-normal fs-6 ms-1">
                            — filtered
                            @if($search)<span class="cl-badge cl-badge-secondary ms-1">{{ $search }}</span>@endif
                            @if(($balance ?? null) === 'with')<span class="cl-badge cl-badge-danger ms-1">With Balance</span>@endif
                            @if(($balance ?? null) === 'without')<span class="cl-badge cl-badge-success ms-1">No Balance</span>@endif
                        </span>
                    @endif
                </span>
            ...
            <table class="cl-table">
                ...
                <tbody>
                    @forelse($customers as $customer)
                        ...
                        @endphp
                        <tr>
                            <td style="padding-left:1.5rem;color:var(--cl-muted);font-size:0.8rem;">{{ $customer->CustomerID }}</td>
                            <td>
                            ...
                            <td style="color:var(--cl-muted);">{{ $customer->contact_number }}</td>
                            <td style="color:var(--cl-muted);max-width:200px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $customer->address }}</td>
                            <td>
                                @if($outstanding > 0)
                                    <span style="color:#dc2626;font-weight:700;">₱{{ number_format($outstanding, 2) }}</span>
                                @else
                                    <span style="color:var(--cl-green);font-weight:600;">₱{{ number_format($outstanding, 2) }}</span>
                                @endif
                            </td>
            ...
            @if($customers->hasPages())
                <div class="d-flex justify-content-between align-items-center px-4 py-3" style="border-top:1px solid var(--cl-border);">
                    <span class="text-muted small">Page {{ $customers->currentPage() }} of {{ $customers->lastPage() }}</span>
                    <div class="d-flex gap-1">
                        @if($customers->onFirstPage())
                            <button class="cl-page-btn" disabled><i class="bi bi-chevron-left"></i></button>
                        @else
                            <a href="{{ $customers->previousPageUrl() }}" class="cl-page-btn"><i class="bi bi-chevron-left"></i></a>
                        @endif
                        @for($i = 1; $i <= $customers->lastPage(); $i++)
                            @if($i == $customers->currentPage())
                                <span class="cl-page-btn active">{{ $i }}</span>
                            @else
                                <a href="{{ $customers->url($i) }}" class="cl-page-btn">{{ $i }}</a>
                            @endif
                        @endfor
                        @if($customers->hasMorePages())
                            <a href="{{ $customers->nextPageUrl() }}" class="cl-page-btn"><i class="bi bi-chevron-right"></i></a>
                        @else
    ...
    <div id="archivedView" style="display: none;">
        <div class="cl-card mt-4" style="padding:0;overflow:hidden;">
            ...
            <table class="cl-table">
                ...
                <tbody>
                    @forelse($archivedCustomers as $customer)
                        @php $initials = strtoupper(substr($customer->First_name, 0, 1) . substr($customer->Last_name, 0, 1)); @endphp
                        <tr>
                            <td style="padding-left:1.5rem;color:var(--cl-muted);font-size:0.8rem;">{{ $customer->CustomerID }}</td>
                            <td>
                                <div class="d-flex align-items-center gap-2">
                                    <div class="cl-avatar" style="background:#9ca3af;">{{ $initials }}</div>
                            ...
                            <td style="color:var(--cl-muted);">{{ $customer->contact_number }}</td>
                            <td style="color:var(--cl-muted);">{{ $customer->address }}</td>
                            <td><span class="cl-badge cl-badge-secondary">Archived</span></td>
                            <td style="text-align:center;padding-right:1.5rem;">
                                <div class="d-flex justify-content-center gap-2">
                                    <form action="{{ route('customers.archive', $customer->CustomerID) }}" method="POST">
                                        @csrf @method('PATCH')
            ...
            @if($archivedCustomers->hasPages())
                <div class="d-flex justify-content-between align-items-center px-4 py-3" style="border-top:1px solid var(--cl-border);">
                    <span class="text-muted small">Page {{ $archivedCustomers->currentPage() }} of {{ $archivedCustomers->lastPage() }}</span>
                    <div class="d-flex gap-1">
                        @if($archivedCustomers->onFirstPage())
                            <button class="cl-page-btn" disabled><i class="bi bi-chevron-left"></i></button>
                        @else
                            <a href="{{ $archivedCustomers->previousPageUrl() }}&tab=archived" class="cl-page-btn"><i class="bi bi-chevron-left"></i></a>
                        @endif
                        @for($i = 1; $i <= $archivedCustomers->lastPage(); $i++)
                            @if($i == $archivedCustomers->currentPage())
                                <span class="cl-page-btn active">{{ $i }}</span>
                            @else
                                <a href="{{ $archivedCustomers->url($i) }}&tab=archived" class="cl-page-btn">{{ $i }}</a>
                            @endif
                        @endfor
                        @if($archivedCustomers->hasMorePages())
                            <a href="{{ $archivedCustomers->nextPageUrl() }}&tab=archived" class="cl-page-btn"><i class="bi bi-chevron-right"></i></a>
                        @else
                            <button class="cl-page-btn" disabled><i class="bi bi-chevron-right"></i></button>
                        @endif
                    </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div style="display:none;" id="addCreditForm">
                    <form action="{{ route('credits.store') }}" method="POST" class="border rounded-3 p-3 mb-3 bg-light">
                        @csrf
                        <input type="hidden" name="customer_id" id="credit_customer_id">
                        {{-- These are synced by JS from the visible inputs before submit --}}
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-3">
                                ...
                                <select class="form-select" name="pump_fuel_id" id="credit_fuel_id">
                                    ...
                                    @isset($fuels)
                                        @foreach($fuels as $pumpFuel)
                                            <option value="{{ $pumpFuel->PumpFuelID }}"
                                                    data-price="{{ $pumpFuel->price_per_liter }}"
                                                    data-name="{{ $pumpFuel->fuel->fuel_name ?? '' }}">
                                                {{ $pumpFuel->pump->pump_name ?? 'Pump' }} — {{ $pumpFuel->fuel->fuel_name ?? '—' }} (₱{{ number_format($pumpFuel->price_per_liter, 2) }}/L)
                                            </option>
                                        @endforeach
                                    @endisset
                                </select>
                            ...
                            {{-- Price per Liter (editable, auto-filled from fuel) --}}
                            <div class="col-md-3">
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                {{-- FIX: field name is 'status' (matches model), not 'payment_status' --}}
                ...
                <div class="mb-3" id="addPaymentSection">
                    ...
                    <div class="collapse mb-3" id="addPaymentForm">
                        {{-- action set dynamically in openCreditDetail() --}}
