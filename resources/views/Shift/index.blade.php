...
const customerOptions = `@foreach($customers as $c)<option value="{{ $c->CustomerID }}">{{ $c->First_name }} {{ $c->Last_name }}</option>@endforeach`;

// Fuel primaryKey is 'FuelID'
const fuelOptions     = `@foreach($fuels as $f)<option value="{{ $f->FuelID }}">{{ $f->fuel_name }}</option>@endforeach`;
