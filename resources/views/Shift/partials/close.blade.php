...
<div class="close-shift-wrap">
    ...
    @if(!$activeShift)
        <div class="alert-card-warn">
            ...
            <div>No active shift to close. <a href="{{ route('shift.management', ['view' => 'open']) }}" style="color:#92400e; font-weight:600;">Open a shift</a> first.</div>
        </div>
    @else
    ...
    <form method="POST" action="{{ route('shift.close') }}" id="close-shift-form">
        ...
        <div class="cs-section">
            <div class="cs-section-head">
                ...
                <div>
                    ...
                    <div class="section-sub">{{ $activeShift->sales_date->format('M d, Y') }} · recorded at shift open</div>
                </div>
            </div>
                ...
                <div class="opening-grid">
                    @foreach($activeShift->shiftReadings as $reading)
                        <div class="opening-pill">
                            <div class="op-label">{{ $reading->pump->pump_name ?? 'Pump' }} — {{ $reading->fuel->fuel_name ?? 'Fuel' }}</div>
                            <div class="op-value">{{ number_format($reading->opening_reading, 3) }} L</div>
                            <input type="hidden" name="opening_readings[{{ $reading->ShiftReadingID }}]" value="{{ $reading->opening_reading }}">
                        </div>
                    @endforeach
                </div>
            </div>
                    ...
                    @foreach($pumps as $pump)
                        <div class="pump-close-card">
                            <div class="pump-close-head">
                                <i class="bi bi-fuel-pump"></i> {{ $pump->pump_name }}
                            </div>
                                ...
                                @foreach($pump->pumpFuels as $pf)
                                    ...
                                    <div class="f-input-group">
                                        ...
                                        <input type="number"
                                               ...
                                               data-label="{{ $pump->pump_name }} — {{ $fuelName }}"
                                               value="{{ old('closing_readings.'.$pf->PumpFuelID) }}"
                                               placeholder="Enter closing reading (L)"
                                               required>
                                        <div class="f-input-hint">Opening: {{ number_format($openingVal, 3) }} L</div>
                                    </div>
                                @endforeach
                            </div>
                        </div>
        ...
        <div class="submit-row">
            ...
            <a href="{{ route('shift.management', ['view' => 'home']) }}" class="btn-cancel-link">Cancel</a>
        </div>
    </form>
